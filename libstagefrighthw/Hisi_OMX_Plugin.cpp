...
OMXPluginBase *createOMXPlugin() {
    return new HisiOMXPlugin;
}

// Original huawei name is libOMX_Core
HisiOMXPlugin::HisiOMXPlugin()
...
OMX_ERRORTYPE HisiOMXPlugin::makeComponentInstance(
        const char *name,
...
OMX_ERRORTYPE HisiOMXPlugin::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    if (mLibHandle == NULL) {
        return OMX_ErrorUndefined;
    }
...
OMX_ERRORTYPE HisiOMXPlugin::enumerateComponents(
        OMX_STRING name,
        size_t size,
        OMX_U32 index) {
    if (mLibHandle == NULL) {
...
OMX_ERRORTYPE HisiOMXPlugin::getRolesOfComponent(
        const char *name,
